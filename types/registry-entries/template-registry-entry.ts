/* eslint-disable */
/**
 * This file was automatically generated by json-schema-to-typescript.
 * DO NOT MODIFY IT BY HAND. Instead, modify the source JSONSchema file,
 * and run json-schema-to-typescript to regenerate this file.
 */

export type Header = {
  [k: string]: unknown;
} & {
  left?: HeaderSection;
  centre?: HeaderSection1;
  right?: HeaderSection2;
  /**
   * Whether to display the header.
   */
  visible?: boolean;
};
/**
 * The left section of the header.
 */
export type HeaderSection =
  | {
      type?: "logo";
      [k: string]: unknown;
    }
  | {
      type?: "menu";
      menu: Menu;
      [k: string]: unknown;
    };
export type Menu = {
  label: string;
  link: string;
  [k: string]: unknown;
}[];
/**
 * The centre section of the header.
 */
export type HeaderSection1 =
  | {
      type?: "logo";
      [k: string]: unknown;
    }
  | {
      type?: "menu";
      menu: Menu;
      [k: string]: unknown;
    };
/**
 * The right section of the header.
 */
export type HeaderSection2 =
  | {
      type?: "logo";
      [k: string]: unknown;
    }
  | {
      type?: "menu";
      menu: Menu;
      [k: string]: unknown;
    };
export type Section = Hero;

/**
 * Metadata for a template in the Bare Site template registry.
 */
export interface BareTemplateRegistryEntry {
  /**
   * The stable identifier of the template.
   */
  id: string;
  /**
   * The display name of the template.
   */
  name: string;
  /**
   * A short description of the template.
   */
  description: string;
  /**
   * The person or organisation that created the template.
   */
  author: string;
  theme?: BareTemplate;
  [k: string]: unknown;
}
/**
 * The schema for a Bare Site template.
 */
export interface BareTemplate {
  /**
   * The version of the Bare Site template schema used by this template
   */
  schemaVersion: 1;
  header: Header;
  footer: Footer;
  /**
   * @minItems 1
   */
  pages: [Page, ...Page[]];
  [k: string]: unknown;
}
export interface Footer {
  /**
   * Whether to display the footer.
   */
  visible?: boolean;
  [k: string]: unknown;
}
export interface Page {
  /**
   * The route of the page.
   */
  route: string;
  /**
   * The title of the page.
   */
  title: string;
  sections: Section[];
  [k: string]: unknown;
}
export interface Hero {
  type: "hero";
  image: string;
  heading: string;
  subheading?: string;
  actions?: Action[];
  [k: string]: unknown;
}
export interface Action {
  label: string;
  link: string;
  style?: "primary" | "secondary";
  [k: string]: unknown;
}
